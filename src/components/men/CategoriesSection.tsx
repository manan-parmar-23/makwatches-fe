...
function CategoryTile({
  ...
  return (
    ...
    >
      <motion.div
        className="group relative aspect-square rounded-2xl overflow-hidden shadow-xl bg-gradient-to-br from-gray-900 to-black h-[140px] md:h-[45vh] w-full md:w-[40vw]"
        ...
        {imageSrc ? (
          <>
            ...
            <div className="absolute top-4 right-4">
              <div className="w-3 h-3 bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-full shadow-lg" />
            </div>
          </>
        ) : (
...
function BrandTile() {
  return (
    <Link href="/">
      <motion.div
        className="group relative aspect-square rounded-2xl overflow-hidden shadow-xl bg-gradient-to-br from-gray-900 via-black to-gray-800 flex items-center justify-center h-[140px] md:h-[45vh] w-full md:w-[40vw]"
        whileHover={{ scale: 1.05, y: -5 }}
      ...
      >
        ...
        <div className="absolute inset-0 opacity-10">
          ...
          <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-yellow-400/10 to-transparent rounded-full -translate-y-16 translate-x-16" />
        </div>
...
function TextTile() {
  return (
    <motion.div
      className="group flex items-center justify-center aspect-square rounded-2xl overflow-hidden shadow-xl bg-gradient-to-br from-yellow-400 to-yellow-600 h-[140px] md:h-[45vh] w-full md:w-[40vw]"
      whileHover={{ scale: 1.05, y: -5, rotate: 1 }}
      transition={{ duration: 0.3 }}
    >
      <div className="text-center">
        <motion.div
          className="text-black font-bold text-lg md:text-2xl uppercase tracking-wider mb-2"
          animate={{ y: [0, -2, 0] }}
          transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
        >
          LUXURY
        </motion.div>
        <div className="text-black/80 text-sm md:text-base font-medium">
          REDEFINED
        </div>
      </div>
      ...
      <div className="absolute bottom-2 left-2 w-1 h-1 bg-black/20 rounded-full" />
    </motion.div>
  );
}
...
function DesktopCategoriesGrid({
  ...
  return (
    <div className="hidden md:grid grid-cols-2 gap-6 min-h-[760px]">
      {first ? (
        <motion.div
          className={`transition-opacity duration-500 ${
            visibleFirst ? "opacity-100" : "opacity-0"
          }`}
          animate={{ opacity: visibleFirst ? 1 : 0 }}
        >
          <CategoryTile category={first} mainCategory="Men" index={0} />
        </motion.div>
      ) : (
        <div className="relative aspect-square rounded-2xl overflow-hidden shadow-xl bg-gradient-to-br from-gray-200 to-gray-300 h-[45vh] w-[40vw] animate-pulse" />
      )}
      <BrandTile />
      <TextTile />
      {second ? (
        <motion.div
          ...
          animate={{ opacity: visibleSecond ? 1 : 0 }}
        >
          <CategoryTile category={second} mainCategory="Men" index={1} />
        </motion.div>
      ) : (
        <div className="relative aspect-square rounded-2xl overflow-hidden shadow-xl bg-gradient-to-br from-gray-200 to-gray-300 h-[45vh] w-[40vw] animate-pulse" />
      )}
  ...
  return (
    <div className="grid grid-cols-2 gap-4 md:hidden">
      {first ? (
        ...
        <div className="relative aspect-square rounded-2xl overflow-hidden shadow-xl bg-gradient-to-br from-gray-200 to-gray-300 h-[140px] animate-pulse" />
      )}
      <BrandTile />
      <TextTile />
      {second ? (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Explore our carefully curated collection of premium timepieces for
            every occasion
          </p>
          <div className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-yellow-600 mx-auto mt-6 rounded-full" />
        </motion.div>
